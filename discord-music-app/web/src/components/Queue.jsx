...
export function Queue({ tracks, onReorder, onRemove, onShuffle, resolutionStats }) {
  ...
  if (tracks.length === 0) {
    return (
      <div className="bg-surface-raised p-6 rounded-lg w-80">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-bold">Up Next</h3>
          <button
            onClick={onShuffle}
            disabled
            className="p-2 text-text-secondary opacity-50 cursor-not-allowed"
            title="Shuffle queue"
        ...
        <p className="text-text-secondary text-center py-8">No upcoming tracks</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-surface-raised p-4 rounded-lg w-80">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <h3 className="text-lg font-bold">Up Next</h3>
          <span className="text-text-secondary text-sm">({tracks.length})</span>
          {hasUnresolved && (
        ...
        <button
          onClick={onShuffle}
          className="p-2 text-text-secondary hover:text-brand-primary transition-colors rounded-full hover:bg-white/10"
          title="Shuffle queue"
          aria-label="Shuffle queue"
        >
          <Shuffle size={20} />
